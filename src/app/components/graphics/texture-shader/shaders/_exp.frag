...
float getOsc(int type, float frequency, float speed, float amplitude, float rotation, vec2 position) {
  float param = frequency * rotateVector(position, rotation).x + speed * _time;
...
vec2 modulatePosition(int type, float frequency, float speed, float amplitude, float rotation, vec2 position) {
  ...
  float x = rotatedVec.x + amplitude * getOscForType(type, rotatedVec.y + _time * frequency);
  return vec2(x, rotatedVec.y);
}

vec2 cloudModulator(vec2 position, float frequency, float speed) {
  return vec2(
    position.x + clouds(position * 0.01 * frequency, _time * 0.1 * speed),
    position.y
  );
}

void main() {
  int _modulatorType = 0;
  int _generatorType = 0;
  vec4 position = gl_FragCoord;

  vec2 modPos = (_modulatorType == 0) ?
    cloudModulator(vec2(position), _modulatorFrequency, _modulatorSpeed) :
    modulatePosition(_modulatorType, _modulatorFrequency, _modulatorSpeed, _modulatorAmplitude, _modulatorRotation, vec2(position));

  float outputVal = (_generatorType == 0) ?
    clouds(modPos * 0.01 * _generatorFrequency, _time * 0.1 * _generatorSpeed) :
    getOsc(_generatorType, _generatorFrequency, _generatorSpeed, _generatorAmplitude, _generatorRotation, modPos);
